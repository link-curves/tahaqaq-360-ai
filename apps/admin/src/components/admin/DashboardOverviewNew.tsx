    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatsCard
            ...
            value={(stats.users?.total || 0).toLocaleString("en-EN")}
          ...
          <StatsCard
            ...
            value={(stats.users?.newThisMonth || 0).toLocaleString("en-EN")}
            icon={Users}
            color="green"
          />
          <StatsCard
            title="نشطون الأسبوع الماضي"
            value={(stats.users?.activeLastWeek || 0).toLocaleString("en-EN")}
            icon={Users}
            color="purple"
          />
          <StatsCard
            title="مستخدمون جدد اليوم"
            value={(stats.users?.newToday || 0).toLocaleString("en-EN")}
            icon={Users}
            color="orange"
          />
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatsCard
            title="الدورات"
            value={(stats.content?.courses?.total || 0).toLocaleString("en-EN")}
          ...
          <StatsCard
            title="الأحداث"
            value={(stats.content?.events?.total || 0).toLocaleString("en-EN")}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">
                  {(stats.users?.newToday || 0).toLocaleString("en-EN")}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">
                  {(stats.content?.faqs?.total || 0).toLocaleString("en-EN")}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
