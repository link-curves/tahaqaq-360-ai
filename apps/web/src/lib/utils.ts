...
/**
 * Verdicts are rows in `veracity_ratings`, not a fixed enum, so these accept a
 * plain string and fall back rather than assuming a closed set. The API also
 * returns an already-resolved `verdict.label`; prefer that for display and use
 * these for styling by code.
 */
export const getVeracityLabel = (rating: string): string => {
...
export const getVeracityColor = (rating: string): string => {
...
// Number formatting
export const formatNumber = (num: number, locale: string = 'en-EN'): string => {
