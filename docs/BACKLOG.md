...
### 8. True bilingual support · 15–25d · **Change request**
The client requires Arabic + English. The schema has **zero** translation columns across 36 models,
and Arabic UI strings are hardcoded in TSX with no i18n library.

This is the **largest gap between what exists and what the client expects**, and it is almost
certainly larger than anyone has priced. See [ADR-0002](./adr/0002-bilingual-content-model.md).

Blocked on two client decisions, not on engineering:
1. Is English at full parity, or a lighter secondary surface?
2. What does the public site show for content translated in only one language?

**Raise this with the client early.** The cost difference between the options is roughly 2×, and the
longer the codebase grows, the more expensive every option becomes.
...
- Nothing here starts without Ali's decision. Items marked **Change request** need the *client's*
